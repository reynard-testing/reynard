...
The following have to be installed locally in order to run a test suite:
...
Preferably, use your IDE to run/debug a specific suite, as this allows you to set breakpoints and investigate what each individual test does. However, if you do want to run it in the terminal, use:

```sh
mvn test -Dtest=<class-name>#<test-method>
# E.g.:
mvn test -Dtest=ExampleSuiteIT#testA
```
