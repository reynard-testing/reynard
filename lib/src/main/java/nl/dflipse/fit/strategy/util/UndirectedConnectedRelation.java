...
public class UndirectedConnectedRelation<X> {
    private final Set<Set<X>> relatedBags = new HashSet<>();

    public void addRelation(X a) {
        if (a == null) {
            throw new IllegalArgumentException("Cannot add null to relation");
        }

        // case 1: a is already in the relation
        if (relatedBags.stream().anyMatch(s -> s.contains(a))) {
            // Do nothing
            return;
        }

        // case 2: a is not in the relation
        relatedBags.add(Set.of(a));
    }
