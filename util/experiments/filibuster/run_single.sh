...
#   WITH_RETRIES: Optional, whether to run with retries opt.  (default: not)
#   WITH_FAULTS: Optional, whether to run with faults         (default: not)
# ------------------------------------------------------------------
...
application_path=${application_path}/${benchmark_id}

# Determine test name and result path
test_name=$(echo "$benchmark_id" | sed -E 's/(^|-)([a-z])/\U\2/g' | tr -d '-')
if [ -n "${WITH_RETRIES}" ]; then
