...
To use the locally built images, instead of those on Docker hub, set the environment variables `USE_REMOTE=false`, `CONTROLLER_IMAGE=fit-controller:latest`, `PROXY_IMAGE=fit-proxy:latest` when running tests.
